...
{
    public static class DropDownTreeForUserExtension
    {
        public static MvcHtmlString DropDownTreeForUser(this HtmlHelper helper, string name, string actionUrl, object defaultValue = null, string callBackFunc = null)
        {
            return DropDownTreeForUserHelper(helper, name, actionUrl, defaultValue, callBackFunc);
        }
        public static MvcHtmlString DropDownTreeForUser<TModel, TProperty>(this HtmlHelper<TModel> helper, Expression<Func<TModel, TProperty>> expression, string actionUrl, string callBackFunc = null)
        {
            ...
            return DropDownTreeForUserHelper(helper, name, actionUrl, value, callBackFunc);
        }

        private static MvcHtmlString DropDownTreeForUserHelper(HtmlHelper helper, string name, string actionUrl, object defaultValue, string callBackFunc = null)
        {
            ...
            htmlBuilder.AppendLine("<input type='text' id='text_{id}' value='{value}' class='form-control' />");
            htmlBuilder.AppendLine("<input type='hidden' id='{id}' name='{id}' value='{value}' />");
            htmlBuilder.AppendLine("<div id='DropDownTreeForUser_{id}' class='dropdown-menu dropdown-tree col-xs-12'>");
            ...
            $(function () {
                $('#btn_{id}').on('click',
                function () {
                    $('#DropDownTreeForUser_{id}').toggle();
                });
