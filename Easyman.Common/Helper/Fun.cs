...
namespace Easyman.Common
{
    public class Fun
    {
        ...
        #region 读取数据库配置文件（读取AppSetting节点）
        /// <summary>
        /// 读取数据库配置文件（读取AppSetting节点）
        /// </summary>
        ...
        /// <param name="strNewExt"></param>
        /// <param name="strMapPath"></param>
        /// <param name="strPath"></param>
        public static void GetPath(ref string strFileName, string strNewExt, ref string strMapPath, ref string strPath)
        {
            if (string.IsNullOrEmpty(strFileName))
                strFileName = string.Format("{0:yyyyMMddHHmmssffff}", DateTime.Now);
            strFileName = strFileName.Trim();
            string strExt = Path.GetExtension(strFileName);
            if (!string.IsNullOrEmpty(strExt))
                strFileName = strFileName.Replace(strExt, string.Empty);
            strFileName += strNewExt;

            if (string.IsNullOrEmpty(strPath))
            {
                // Logger.Debug("SaveFile值为空，停止保存导出的文件");
                return;
            }
            strPath += "/" + DateTime.Now.Month + "/";
            strMapPath = System.Web.HttpContext.Current.Server.MapPath(strPath);
