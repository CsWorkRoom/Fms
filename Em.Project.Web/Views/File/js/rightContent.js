...
function loadHtml(dataArr, nodeType) {
    ...
    if (dataArr != null && dataArr.length > 0) {
        for (var i = 0; i < dataArr.length; i++) {
            ...
            switch (nodeType) {
                case "first":
                    htm += '<div class="col-sm-2" style="cursor:pointer;float:left;width:100px;height:100px;margin-left:35px;margin-bottom:20px;word-wrap:break-word;overflow:hidden;text-align:center" ondblclick="openModel(' + dataArr[i].id.substring(dataArr[i].id.indexOf("_") + 1) + ',\'computer\',false)"  title="' + dataArr[i].name + '"><span ><i class="glyphicon glyphicon-folder-close" style="font-size:2em; "></i><br /><span class="text-overflow">' + sname + '</span></span></div>';
                    break;
                case "computer":
                    htm += '<div class="col-sm-2" style="cursor:pointer;float:left;width:100px;height:100px;margin-left:35px;margin-bottom:20px;word-wrap:break-word;overflow:hidden;text-align:center" ondblclick="openModel(' + dataArr[i].id + ',\'folder\',false)"  title="' + dataArr[i].name + '"><span><i class="glyphicon glyphicon-folder-close" style="font-size:2em;"></i><br /><span class="text-overflow">' + sname + '</span></span></div>';
                    break;
                case "folder":
                    if (dataArr[i].isHide)
                    {
