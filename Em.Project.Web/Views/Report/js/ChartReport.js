//**********************************************
//该页面仅支持PC端Echart报表的展示
//**********************************************

$(document).ready(function () {
    var chartReport = InitChartReport();//解析CHART报表
    WinResize();
    //终止事件冒泡
...
//获取当前PC端CHART报表
//childRpType:1=CHART 2键值 3=图形 4=chart
//appType:PC、APP
...
//解析CHART报表
//不支持外置事件
function InitChartReport() {
    var fieldArr = $.parseJSON("[]"); //字段信息初始化
    var chartReport = GetChildReport(3,"PC");//获取当前PC端CHART报表
    ...
    //获取筛选集合 filterHts
    if (chartReport.FilterListJson != null && chartReport.FilterListJson != "" && chartReport.FilterListJson != "[]") {
        var filterArr = $.parseJSON(chartReport.FilterListJson);
    ...
    var strTools = "";
    if (chartReport.InEventListJson != null && chartReport.InEventListJson != "" && chartReport.InEventListJson != "[]")
    {
        var inEvList = $.parseJSON(chartReport.InEventListJson);
    ...
    var queryParamArr = GetQueryParamArr(chartReport);//获得查询参数集合列表
    return chartReport;
}
...
function GridExplain() {
    var strTitle = "报表说明";
    var chartReport = GetChildReport(3,"PC");
    var strContent = (chartReport.Remark != undefined && chartReport.Remark != null && chartReport.Remark != "" && chartReport.Remark.toLowerCase() != "null" && breport.Remark != "NaN" ? chartReport.Remark : "暂无！");
...
function DoSearch() {
    var chartReport = GetChildReport(3, "PC");
    //$("#ifm").contents().find("#chartJsCode").append(chartReport.EndCode);
    $("#ifm").contents().find("#endCode").val(Encrypt(chartReport.EndCode));


    if (chartReport != "" && chartReport != null) {
        var queryParamArr = GetQueryParamArr(chartReport);//获得查询参数集合列表

        var data = $.ajax({
            type: "post",
            data: { code: $("#Code").val(), queryParams: JSON.stringify(queryParamArr) },
            url: "GetChartData",
            async: false
        });
...
//获得查询参数集合列表
function GetQueryParamArr(chartReport) {
